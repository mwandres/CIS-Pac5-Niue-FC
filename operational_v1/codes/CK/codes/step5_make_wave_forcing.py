...
def make_waves(now):
    ...
    f14bnds = pd.read_csv('../extras/rarotonga/swan/f14_boundary_points.csv',sep=",")
    f14_xb = np.array(f14bnds["x"])
    f14_yb = np.array(f14bnds["y"])
    f14_id = np.array(f14bnds["index"])
    ...
    for l in range(len(f14_xb)):
        ...
        out_nameXX=out_name+'/sp_' + "{:03d}".format(f14_id[l])  
