...
# Function to read Sea Level Pressure  contained in the netCDF files from step1_download_NCEP.py
    ...
    slp = np.nan_to_num(np.array(nc['Pressure_surface'])[:,1,1])# Position 1,1 is close to Niue but not impacted by the land
   ...
   # read SLP from hour=3 to hour=180 every 3 hours
    for i in range(dtt,length_dt,dtt):
        ...
        tt = np.append(tt,tt_)
        slp = np.nan_to_num(np.append(slp,np.array(nc['Pressure_surface'])[:,1,1],axis=0))
        print(nc_fnameX)
